...
public final class EasyClient {
	...
	/**
	 * Requirements for this method: innerHandle has a host A composition has been
	 * created.
	 */
	...
	public JASEDataObject invokeOneLineOperation(final String methodName, final String... methodArgNames)
			throws IOException, InterruptedException {
		// check if requirements met
		{
			if (!this.innerHandle.isRemote()) {
	...
	/**
	 * Appends a construct operation to the composition. Requires a host to be set
	 * before.
	 *
	 * @param outFieldName
	 ...
	 */
	public EasyClient withAddedConstructOperation(final String outFieldName, final String classpath,
			final String... constructorArgFieldNames) {
		...
		this.body.addOpToComposition(compositionLine);
		return this;
	}

	/**
	 * Appends a method operation to the composition. Additionally, if the
	 * composition was empty before adding the created operation, the innerHandle
	 * will be set to the service retrieved with 'variableFieldName' from the
	 * keyword argument pool. If the service has not been added with
	 ...
	 *
	 */
	public EasyClient withAddedMethodOperation(final String outFieldName, final String variableFieldName,
			final String methodName, final String... methodArgFieldNames) {
		...
		if (!this.body.containsComposition()) {
			...
			if (this.body.getState().containsField(variableFieldName)) {
				JASEDataObject jdo = this.body.getState().retrieveField(variableFieldName);
				if (jdo.isofType(ServiceHandle.class.getSimpleName())) {
					this.withService((ServiceHandle) jdo.getData()); // replace the
					// inner handler
				}
			}
		}
	...
	public EasyClient withKeywordArgument_StringList(final String keyword, final String... argValues) {
		...
		List<String> stringlist = new ArrayList<>(Arrays.asList(argValues));
		return this.withKeywordArgument(keyword, stringlist);
	}
	...
	public EasyClient withPositionalArgument_StringList(final String... argValues) {
		...
		return this.withPositionalArgument(stringlist);
	}
	 ...
	 */
	public EasyClient withOTMS(final OntologicalTypeMarshallingSystem otms) {
		if (otms != null) {
			this.otms = otms;
		}
		return this;
	...
	public String getCurrentCompositionText() {
		return this.body.getComposition();
	}
