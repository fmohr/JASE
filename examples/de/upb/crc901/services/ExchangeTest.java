...
public class ExchangeTest {
  ...
  public void setup() {
    ...
    this.instances = new SimpleInstancesImpl();
    this.stringList = new ArrayList<>();
    this.linstances = new SimpleLabeledInstancesImpl();
    for (int j = 0; j < 2; j++) {
      this.instance = new SimpleInstanceImpl();
      this.linstance = new SimpleLabeledInstanceImpl();
      for (int i = 0; i < 3; i++) {
        this.instance.add((double) i + j * 20);
        this.linstance.add((double) i + j * 20);
      }
      this.linstance.setLabel("label" + j);
      this.stringList.add("label" + j);
      this.instances.add(this.instance);
      this.linstances.add(this.linstance);
    }
  }

  // @Test
  public void basicTest() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, ClassNotFoundException, InstantiationException, NoSuchMethodException,
      SecurityException, IOException {
    HttpBody body = new HttpBody();
    body.setComposition("something");
    body.setCurrentIndex(2);
    body.addKeyworkArgument("a", new JASEDataObject("Instances", this.instances));
    body.addUnparsedKeywordArgument("i1", 1);
    body.addPositionalArgument(new JASEDataObject("LabeledInstances", this.linstances));
  ...
  @Test
  public void timedTest() throws IOException, InterruptedException {
    // List<weka.core.Instances> split = WekaUtil.getStratifiedSplit(wekaInstances, new Random(0),
    // .01f);
